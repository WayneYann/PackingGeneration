...
do
	...
	then
		...
		surfaceTransformPoints -rotate "( (0 1 0) ($xR $yR 0) )" sphere1.stl sphere2.stl > log
		surfaceTransformPoints -translate "($xk $yk $zi)" sphere2.stl sphere3.stl > log
	...
	do
		...
		then
			...
			then
				...
				xM=$(echo "($xi+$xk)*0.5" | bc -l)
				yM=$(echo "($yi+$yk)*0.5" | bc -l)
				zM=$(echo "($zi+$zk)*0.5" | bc -l)
				xR=$(echo "($xi - $xk)" | bc -l)
				yR=$(echo "($yi - $yk)" | bc -l)
				zR=$(echo "($zi - $zk)" | bc -l)
				#Move and resize
				surfaceTransformPoints -scale "($rb $hb $rb)" ../mandatoryFiles/startingBridge.stl sphere1.stl > log
				surfaceTransformPoints -rotate "( (0 1 0) ($xR $yR $zR) )" sphere1.stl sphere2.stl > log
				surfaceTransformPoints -translate "($xM $yM $zM)" sphere2.stl sphere3.stl > log
				sed "/^solid/d" sphere3.stl | sed "/^endsolid/d" >> bridges.stl 
				rm -f log
				a=$(echo "$a+1" | bc -l)
				if [ $a -gt $b_per_block ]
